...
submission_df = pd.read_csv("v1_full_submission.csv")
ground_truth_df = pd.read_csv("shopee-product-matching/train.csv")

# Map: label_group → list of posting_ids
gt_dict = (
    ground_truth_df
    .groupby('label_group')['posting_id']
    .apply(list)
    .to_dict()
)

# Map: posting_id → true set of matches
true_labels = {
    pid: set(gt_dict[label]) for pid, label in zip(ground_truth_df['posting_id'], ground_truth_df['label_group'])
}

# Full predictions (for uncapped F1)
pred_labels_full = {
    row['posting_id']: set(row['matches'].split())
    for _, row in submission_df.iterrows()
}

# Top-50 predictions (for Recall@50)
pred_labels_top50 = {
...
for pid in submission_df['posting_id']:
    pred_set_full = pred_labels_full[pid]
